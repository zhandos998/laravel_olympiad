...
export function StudentOlympiadTestingPage() {
    ...
    const copy = useMemo(() => {
        if (locale === "kaz") {
            return {
                ...
                proctoringTitle:
                    "\u041f\u0440\u043e\u043a\u0442\u043e\u0440\u0438\u043d\u0433",
                proctoringStart:
                    "\u0420\u04b1\u049b\u0441\u0430\u0442 \u0431\u0435\u0440\u0456\u043f, \u043f\u0440\u043e\u043a\u0442\u043e\u0440\u0438\u043d\u0433\u0442\u0456 \u0431\u0430\u0441\u0442\u0430\u0443",
                proctoringRetry:
                    "\u041f\u0440\u043e\u043a\u0442\u043e\u0440\u0438\u043d\u0433\u0442\u0456 \u049b\u0430\u0439\u0442\u0430 \u049b\u043e\u0441\u0443",
                combinedLabel:
                    "\u0411\u0456\u0440\u0456\u043a\u043a\u0435\u043d \u0436\u0430\u0437\u0431\u0430",
            };
        }
        ...
        return {
            ...
            proctoringTitle:
                "\u041f\u0440\u043e\u043a\u0442\u043e\u0440\u0438\u043d\u0433",
            proctoringStart:
                "\u0420\u0430\u0437\u0440\u0435\u0448\u0438\u0442\u044c \u0434\u043e\u0441\u0442\u0443\u043f \u0438 \u0437\u0430\u043f\u0443\u0441\u0442\u0438\u0442\u044c \u043f\u0440\u043e\u043a\u0442\u043e\u0440\u0438\u043d\u0433",
            proctoringRetry:
                "\u0417\u0430\u043d\u043e\u0432\u043e \u0432\u043a\u043b\u044e\u0447\u0438\u0442\u044c \u043f\u0440\u043e\u043a\u0442\u043e\u0440\u0438\u043d\u0433",
            combinedLabel:
                "\u041e\u0431\u0449\u0430\u044f \u0437\u0430\u043f\u0438\u0441\u044c",
        };
    }, [locale]);
    ...
    const loadOlympiad = async () => {
        try {
            const response = await api(`/student/olympiads/${olympiadId}`, {
                token,
            });
            setData(response);
            setError("");
    ...
    const remainingSeconds = data?.registration?.stage1_ends_at
        ? Math.max(
              Math.floor(
                  (new Date(data.registration.stage1_ends_at).getTime() - now) /
                      1000
              ),
              0
          )
    ...
    const loadSubjectAttempt = async (subjectId, questionIndex = 0) => {
        try {
            const response = await api(
                `/student/subjects/${subjectId}/stage-one/start`,
                {
                    method: "POST",
                    token,
                }
    ...
    useEffect(() => {
        ...
        const nextSubject =
            data.subjects.find(
                (subject) => subject.stage1_attempt?.status !== "completed"
            ) ?? null;
    ...
    useEffect(() => {
        if (
            remainingSeconds !== 0 ||
            !data?.registration?.stage1_started_at ||
            isFinishing
        ) {
            return;
        }
    ...
    const activeQuestion =
        activeAttempt?.questions?.[currentQuestionIndex] ?? null;
    const activeSubject =
        data?.subjects.find(
            (subject) => String(subject.id) === String(activeSubjectId)
        ) ?? null;
    ...
    const getQuestionCount = (subject) =>
        attemptsBySubject[String(subject.id)]?.questions?.length ??
    ...
    const getNextSubjectInOrder = (subjectId, sourceData = data) => {
        ...
        const subjects = sourceData.subjects;
    ...
    const updateAnswer = async (subjectId, questionId, optionId) => {
        ...
        const previousOptionId =
            answersBySubject[normalizedSubjectId]?.[questionId] ?? "";
    ...
    const submitSubject = async (subjectId, options = {}) => {
        ...
        const answers =
            answersBySubject[normalizedSubjectId] ??
            buildAnswerMap(attempt.questions);

        try {
            const response = await api(
                `/student/subjects/${subjectId}/stage-one/submit`,
                {
                    method: "POST",
                    token,
                    body: {
                        answers: attempt.questions.map((question) => ({
                            ...
                            option_id: answers[question.question_id]
                                ? Number(answers[question.question_id])
                                : null,
                        })),
                    },
                }
            ...
            if (options.refresh !== false) {
                ...
                if (!options.silent && refreshed) {
                    const nextSubject = refreshed.subjects.find(
                        (subject) =>
    ...
    async function finishOlympiad() {
        ...
        try {
            for (const subject of data.subjects) {
                ...
                const submitted = await submitSubject(subject.id, {
                    silent: true,
                    refresh: false,
                });
        ...
        <div className="rounded-[1.5rem] border border-slate-200 bg-white/95 p-4 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
                    {copy.completedSubjects}: {completedSubjectsCount}/
                    {data.subjects.length}
                </span>
            </div>

            <div className="mt-4 grid gap-3 overflow-y-auto pr-1 xl:max-h-[32vh]">
                {data.subjects.map((subject) => {
                    ...
                    const isActiveSubject =
                        subjectId === String(activeSubjectId);
                        ...
                        >
                            <div className="flex flex-wrap items-start justify-between gap-3">
                                ...
                                >
                                    {isCompleted
                                        ? copy.subjectDone
                                        : `${copy.answered}: ${subjectAnsweredCount}/${questionCount}`}
                                </span>
                            </div>
                    ...
                    <div className="mt-4 rounded-[1.25rem] border border-slate-300 bg-white p-3 shadow-sm">
                        ...
                        <div className="mt-3 overflow-hidden rounded-[1rem] border border-slate-200 bg-slate-950">
                            {webcamStream ? (
                                ...
                                <div className="flex h-32 items-center justify-center px-3 text-center text-xs text-slate-300">
                                    {proctoringMessage}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="rounded-[1.5rem] border border-slate-200 bg-white/95 p-4 shadow-sm md:p-5">
                            <div className="flex flex-col gap-3 border-b border-slate-200 pb-4 md:flex-row md:items-start md:justify-between">
                                ...
                                <div className="grid gap-1 rounded-2xl bg-slate-50 px-3 py-3 text-sm text-slate-700">
                                    <p>
                                        {copy.answered}: {activeAnsweredCount}/
                                        {activeAttempt.questions.length}
                                    </p>
                                    <p>
                                        {copy.unanswered}:{" "}
                        ...
                        <div className="flex min-h-[320px] items-center justify-center rounded-[1.5rem] border border-dashed border-slate-300 bg-white/80 p-6 text-center shadow-sm">
                            <div className="max-w-md">
                                ...
                                <h3 className="mt-3 text-2xl font-extrabold text-slate-900">
                                    {copy.noActiveSubject}
                                </h3>
                                <p className="mt-3 text-sm leading-6 text-slate-600">
                                    {copy.noActiveSubjectDescription}
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="w-full max-w-lg rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-[0_30px_90px_-35px_rgba(15,23,42,0.5)]">
                        ...
                        <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:justify-end">
                            <button
                                ...
                                type="button"
                            >
                                {proctoringStatus === "idle"
                                    ? copy.proctoringStart
                                    : copy.proctoringRetry}
                            </button>
                        </div>
                    </div>
                </div>
